...
public class CLH_Miner {
	...
	public void runAlgorithm(int minUtil, String inputPath, String outputPath, String TaxonomyPath) throws IOException {
		...
		try {
			...
			SearchTree(new int[0], 0, null, listUtilityLevel1);
		} catch (Exception e) {
	...
	private void SearchTree(int[] prefix, int prefixLength, UtilityList pUL, List<UtilityList> ULs) {
...
//				System.out.println("  #UTIL: " + X.sumIutils);
			...
			SearchTree(newPrefix, prefixLength + 1, X, exULs);
		}
			...
			for (Element PElment : P.getElement()) {
				...
				if (UnionChild != null) {
					...
					for (int i = 0; i < trans.size(); i++) {
						...
						if (compareItems(currentItem, Child.item) < 0 && compareItems(currentItem, prefix[prefix.length-1]) > 0 &&(!CheckParent(Child.item, currentItem))&&
			...
			if (P == null) {
				...
				if (e != null) {
					...
					for (int i = 0; i < trans.size(); i++) {
						Integer currentItem = trans.get(i).item;
						if (compareItems(currentItem, py.item) < 0 && compareItems(currentItem, px.item) > 0&&
								(!CheckParentWithPrefix(currentItem,prefix))&& (!CheckParent(px.item, currentItem)) && (!CheckParent(py.item, currentItem))) {
							remainUtility -= trans.get(i).utility;
						}
					}
